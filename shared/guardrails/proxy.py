...
def _parse_trusted_cidrs(spec: str) -> list[ipaddress.IPv4Network | ipaddress.IPv6Network]:
    ...
    for raw in spec.split(","):
        ...
        try:
            ...
            if "/" not in raw:
                # Convert bare IP to /32 (v4) or /128 (v6) network
...
def _ip_in_trusted(
    ...
    """Check whether a given IP string is in any of the trusted networks."""
    try:
...
class TrustedProxyMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if peer_ip and _ip_in_trusted(peer_ip, self.trusted_networks):
            ...
            if forwarded:
                # Take the leftmost address — the original client.
                client_ip = forwarded.split(",")[0].strip()
            else:
                client_ip = peer_ip
        else:
            # Peer is not a trusted proxy — ignore any X-Forwarded-For header.
            client_ip = peer_ip
